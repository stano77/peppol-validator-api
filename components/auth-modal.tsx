...
export function AuthModal({ open, onOpenChange }: AuthModalProps) {
  ...
  const signIn = async (provider: "google" | "apple") => {
    // Store a flag that we have a pending validation
    localStorage.setItem("pendingValidation", "true")

    await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: `${window.location.origin}/auth/callback?pendingValidation=true`,
      },
    })
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md glass-heavy border-glass-border" showCloseButton={true}>
        ...
        <div className="flex flex-col gap-3 pt-4">
          <Button
            variant="outline"
            className="w-full gap-3 h-12 glass-subtle border-glass-border hover:bg-accent transition-all duration-200"
